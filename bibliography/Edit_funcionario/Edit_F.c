...
void edit_funcionario(){
    ...
    snprintf(nome_arquivo, sizeof(nome_arquivo), "C:\\Users\\jhfcs\\Desktop\\CadStore\\data\\funcionario_%s.txt", cpf);

    // Verifica se o arquivo existe
    FILE *arquivo = fopen(nome_arquivo, "r");
    if(arquivo == NULL){
        printf("\n\tFuncionario com CPF %s não encontrado.\n", cpf);
        return;
    }
    fclose(arquivo);
